...
export const createServer = ({ dir, port = 3000, token, tlsCert, tlsKey, publicUrl, trustProxy }) => {
  ...
  const authorize = (req) => {
    ...
    const id = t.slice(0, sep)
    const secret = t.slice(sep + 1)
    return store.authenticate(id, secret) ? id : null
  }
  ...
  const handler = async (req, res) => {
    ...
    if (req.method === 'POST') {
      if (path === '/api/register') {
        ...
        try {
          const result = store.register(body.id, body.token)
      ...
      if (path === '/api/login') {
        let body
        try { body = JSON.parse(await readBody(req, 4096)) } catch { return json(res, 400, { ok: false, error: 'invalid json' }) }
        if (!body.id || !body.token) return json(res, 400, { ok: false, error: 'id and token required' })
        try {
          const valid = store.authenticate(body.id, body.token)
          if (!valid) return json(res, 401, { ok: false, error: 'invalid id or token' })
  ...
  server.listen(port, '0.0.0.0', () => {
    const scheme = tlsCert ? 'https' : 'http'
    console.error(`pagesite: serving ${dir} on ${scheme}://0.0.0.0:${port}`)
  })
  return server
}
