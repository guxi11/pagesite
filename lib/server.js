...
export const createServer = ({ dir, port = 3000, token, tlsCert, tlsKey, publicUrl, trustProxy }) => {
  ...
  const handler = async (req, res) => {
    ...
    if (req.method === 'POST') {
      ...
      if (path === '/api/login') {
        ...
        try {
          ...
          const apiKey = store.getApiKey(body.id)
    ...
    if (req.method === 'GET') {
      ...
      if (path === '/api/list') {
        ...
        if (!userId) return json(res, 403, { ok: false, error: 'forbidden' })
        const owned = new Set(store.listUserPages(userId))
        const files = readdirSync(dir)
          .filter(f => /\.html?$/.test(f) && (userId === '__admin__' || owned.has(f)))
          .map(f => {
    ...
    if (req.method === 'PUT') {
      ...
      store.setPageOwner(name, userId)
    ...
    if (req.method === 'DELETE') {
      ...
      const userId = authorize(req)
      if (!userId) return txt(res, 403, 'forbidden')
      const owner = store.getPageOwner(name)
      if (userId !== '__admin__' && owner && owner !== userId) return txt(res, 403, 'forbidden')
      try {
        unlinkSync(join(dir, name))
        store.removePageMeta(name)
  ...
  server.listen(port, '0.0.0.0', () => {
    const scheme = tlsCert ? 'https' : 'http'
    console.error(`pagest: serving ${dir} on ${scheme}://0.0.0.0:${port}`)
  })
  return server
}
